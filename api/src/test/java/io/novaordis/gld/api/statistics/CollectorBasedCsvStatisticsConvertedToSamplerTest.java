...
import io.novaordis.gld.api.sampler.MockSamplingConsumer;
...
public class CollectorBasedCsvStatisticsConvertedToSamplerTest {
    ...
    public void lifeCycleIntegrationTest() throws Exception {
        ...
        MockSamplerOperation mo = new MockSamplerOperation();
        ...
        assertEquals("MockSamplerOperation Success Rate (ops/sec)", token);

        assertTrue(lineTokenizer.hasMoreTokens());
        token = lineTokenizer.nextToken().trim();
        assertEquals("MockSamplerOperation Average Duration (ms)", token);

        assertTrue(lineTokenizer.hasMoreTokens());
        token = lineTokenizer.nextToken().trim();
        assertEquals("MockSamplerOperation Failure Rate (ops/sec)", token);
